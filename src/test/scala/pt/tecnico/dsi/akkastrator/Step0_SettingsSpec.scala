...
object Step0_SettingsSpec {
  val loggersConfig = ConfigFactory.load()
    .withValue("akka.loggers", ConfigValueFactory.fromIterable(List("akka.testkit.TestEventListener").asJava))
...
class Step0_SettingsSpec extends ActorSysSpec(Some(Step0_SettingsSpec.loggersConfig)) {
  ...
  "useTaskColors" when {
    "set to true" should {
      "cause the log messages to have console colors" in {
    ...
    "set to false" should {
      "cause the log messages to NOT have console colors " in {
